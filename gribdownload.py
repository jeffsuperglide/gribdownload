...
"""
    ...
    parser_qpf.add_argument(
        ...
        action="store",
        type=int,
        default=datetime.datetime.utcnow().strftime("%H"),
        metavar="h",
    ...
    parser_hrrr.add_argument(
        ...
        default=0,
        metavar="0",
        help="Define the left longitude; Use negative numbers for south and west; Default=%(default)s",
        dest="llon",
    )
    parser_hrrr.add_argument(
        "--right-lon",
        ...
        default=360,
        metavar="360",
        help="Define the right longitude; Use negative numbers for south and west; Default=%(default)s",
        dest="rlon",
    )
    parser_hrrr.add_argument(
        "--top-lat",
        ...
        default=90,
        metavar="90",
        help="Define the top latitude; Use negative numbers for south and west; Default=%(default)s",
        dest="tlat",
    )
    parser_hrrr.add_argument(
        "--bottom-lat",
        ...
        default=-90,
        metavar="-90",
        help="Define the bottom latitude; Use negative numbers for south and west; Default=%(default)s",
        dest="blat",
    )
...
def match_files(localfiles, url, regex, force):
    ...
    files.sort()
    urls = [urljoin(baseurl, file) for file in files]
    return files, urls
...
def download(q, outdir, index):
    ...
    while not q.empty():
        ...
        try:
            ...
            logger.info("Thread {} - Download saved to: {}".format(index, outputfile))
        except IOError as ex:
            logger.warning(ex)
        except KeyError:
...
"""
if not os.path.isdir(args.output_dir):
    logger.warning('"{}" not a directory'.format(args.output_dir))
    logger.warning("Script Exiting!")
    sys.exit(1)
...
if args.subparser_name == "qpe":
    ...
    baseurl = mrms_base.format(
        PRODUCT=_prod, HOUR="{:02}".format(args.interval), PASS=_pass
    )
    regex = r">(MRMS\w*\.\d*_\d*-\d*\.grib2\.gz)<"
    ...
    baseurl = qpf_base
    regex = r">(p{:02}m_......{}{}f\d\d\d\.grb)<".format(args.interval, dy, args.cycle)
    files, urls = match_files(local_files, baseurl, regex, args.force)
